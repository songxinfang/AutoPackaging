...
- (id)initWithView:(UIView *)view {
    ...
    if ([view isKindOfClass:[UIWindow class]]) {
        return [self initWithFrame:CGRectMake(0, 64, SCREEN_WIDTH, SCREEN_HEIGHT - 64)];
    }
...
- (void)layoutSubviews {
    ...
    CGSize labelSize = [label.text boundingRectWithSize:CGSizeMake(maxWidth, 300) options: NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
    //    MFLog(@"labelSize =%@",NSStringFromCGSize(labelSize));
    //    MFLog(@"labelText =%@",label.text);
