...
@interface MyExamViewController ()
<
...
{
    self.dataSource = nil;
    NSArray *data = [[QuestionsApi share] existingExaminationProperty];
...
{
    [CustomAlertView showAlertWithTitle:@"考试记录清除后无法恢复，确认是否要清除？"
                                message:nil
                            cancelTitle:@"取消"
                             otherTitle:@"删除" completion:^(BOOL cancelled, NSInteger buttonIndex) {
                                 if (!cancelled) {
                                     // 1 从数据库清除
                                     [[QuestionsApi share] clearExaminationRecord];
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.m_ScoreLabel setAttributedText:[self attributedWithLeft:nil
                                                           middle:[NSString stringWithFormat:@"%ld",property.Person_score]
                                                            right:@"分"]];
    
    [cell.m_RankLabel setAttributedText:[self attributedWithLeft:@"第"
                                                           middle:[NSString stringWithFormat:@"%ld",property.ranking]
                                                            right:@"名"]];
    ...
    cell.m_TitleLabel.width = SCREEN_WIDTH - 140 - cell.m_TitleLabel.x;
...
- (NSMutableAttributedString *)attributedWithLeft:(NSString *)leftStr middle:(NSString *)middleStr right:(NSString *)rightStr
{
    NSString *content = middleStr;
    
    if (leftStr) {
        content = [NSString stringWithFormat:@"%@%@",leftStr,content];
    }

    if (rightStr) {
        content = [NSString stringWithFormat:@"%@%@",content,rightStr];
    }
    ...
    if (leftStr.length) {
        NSRange leftRange = [content rangeOfString:leftStr];
