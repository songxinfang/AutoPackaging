...
@interface CustomAlertViewStack : NSObject

@property (nonatomic) NSMutableArray *alertViews;

+ (CustomAlertViewStack *)sharedInstance;

- (void)push:(CustomAlertView *)alertView;
- (void)pop:(CustomAlertView *)alertView;

@end

static const CGFloat AlertViewWidth = 280.0;
static const CGFloat AlertViewContentMargin = 30;
static const CGFloat AlertViewVerticalElementSpace = 25;
static const CGFloat AlertViewButtonHeight = 45;
static const CGFloat AlertViewLineLayerWidth = 0.5;
static const CGFloat AlertViewVerticalEdgeMinMargin = 25;

NSString *AlertTitleFontName = @"HelveticaNeue-Light";
static const CGFloat AlertTitleFontSize = 16.0;
static const CGFloat AlertMessageFontSize = 14.0;
static const CGFloat AlertButtonFontSize = 14.0;
         ...
         completion:(CustomAlertViewCompletionBlock)completion
{
    ...
    if (self) {
        ...
        self.titleLabel.font = [UIFont fontWithName:AlertTitleFontName size:AlertTitleFontSize];
        self.titleLabel.numberOfLines = 0;
        
        if (![self isSingleLine:self.titleLabel]) {
            if (title) {
        ...
        self.messageLabel.textColor = [UIColor colorWithHex:0x333333];
        self.messageLabel.font = [UIFont fontWithName:AlertTitleFontName size:AlertMessageFontSize];
        self.messageLabel.numberOfLines = 0;
        
        if (![self isSingleLine:self.messageLabel]) {
            if (message) {
                NSMutableAttributedString *messageAttributedString = [[NSMutableAttributedString alloc] initWithString:message];
                [messageAttributedString addAttributes:@{NSFontAttributeName : self.messageLabel.font,
                                                         NSParagraphStyleAttributeName : [self paragraphStyle]}
        ...
        if (self.messageScrollView.frame.size.height == 0)
        {
            lineLayer.frame = CGRectMake(0, self.messageScrollView.frame.origin.y, AlertViewWidth, AlertViewLineLayerWidth);
        }
...
{
    static NSMutableParagraphStyle *_paragraphStyle = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _paragraphStyle = [NSMutableParagraphStyle new];
        _paragraphStyle.alignment = NSTextAlignmentCenter;
        _paragraphStyle.lineSpacing = 12.0;//调整行间距
    });
    
    return _paragraphStyle;
}

- (BOOL)isSingleLine:(UILabel *)label
{
    // 判断文本是否只有一行
...
- (CGRect)adjustLabelFrameHeight:(UILabel *)label
{
    ...
    CGRect bounds = [label.text boundingRectWithSize:CGSizeMake(label.frame.size.width, FLT_MAX)
                                          ...
                                          attributes:@{NSFontAttributeName : label.font}
                                             context:nil];
    if (bounds.size.height < 20) {
        return CGRectMake(label.frame.origin.x, label.frame.origin.y, label.frame.size.width, bounds.size.height);
    }
    
    bounds = [label.text boundingRectWithSize:CGSizeMake(label.frame.size.width, FLT_MAX)
...
- (UIButton *)genericButton
{
    ...
    button.titleLabel.font = [UIFont fontWithName:AlertTitleFontName size:AlertButtonFontSize];
    button.titleLabel.adjustsFontSizeToFitWidth = YES;
    button.titleEdgeInsets = UIEdgeInsetsMake(2, 2, 2, 2);
    [button setTitleColor:[UIColor colorWithHex:0x6F80C8] forState:UIControlStateNormal];
...
{
    [sender setBackgroundColor:[UIColor colorWithHex:0xFAFAFA]];
}
...
- (void)show
{
    [[CustomAlertViewStack sharedInstance] push:self];
}
...
- (void)dismiss:(id)sender animated:(BOOL)animated
{
    ...
    [UIView animateWithDuration:(animated ? 0.2 : 0) animations:^{
        ...
        if ([[[CustomAlertViewStack sharedInstance] alertViews] count] == 1) {
            if (animated) {
        ...
        [[CustomAlertViewStack sharedInstance] pop:self];
    }];
}
...
- (CALayer *)lineLayer
{
    ...
    lineLayer.backgroundColor = [[UIColor colorWithHex:0xEAEAEA] CGColor];
    return lineLayer;
}
...
{
    return [[self class] showAlertWithTitle:title message:nil cancelTitle:@"确定" completion:nil];
}

+ (instancetype)showAlertWithTitle:(NSString *)title
                           message:(NSString *)message
{
    return [[self class] showAlertWithTitle:title message:message cancelTitle:@"确定" completion:nil];
}
...
+ (instancetype)showAlertWithTitle:(NSString *)title
                        ...
                        completion:(CustomAlertViewCompletionBlock)completion
{
    return [[self class] showAlertWithTitle:title message:message cancelTitle:@"确定" completion:completion];
}
...
+ (instancetype)showAlertWithTitle:(NSString *)title
                       ...
                       cancelTitle:(NSString *)cancelTitle
                        completion:(CustomAlertViewCompletionBlock)completion
{
    CustomAlertView *alertView = [[self alloc] initWithTitle:title
                                                 message:message
                                             cancelTitle:cancelTitle
                                              otherTitle:nil
...
- (NSInteger)addButtonWithTitle:(NSString *)title
{
    ...
    if (!self.cancelButton) {
        ...
        lastButton.titleLabel.font = [UIFont fontWithName:AlertTitleFontName size:AlertButtonFontSize];
        ...
        CALayer *lineLayer = [self lineLayer];
        lineLayer.frame = CGRectMake(0, self.buttonsY + AlertViewButtonHeight*[self.buttons count], AlertViewWidth, AlertViewLineLayerWidth);
        [self.alertView.layer addSublayer:lineLayer];
        ...
        self.cancelButton.titleLabel.font = [UIFont fontWithName:AlertTitleFontName size:AlertButtonFontSize];
    }
...
@implementation CustomAlertViewStack

+ (instancetype)sharedInstance
{
    static CustomAlertViewStack *_sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[CustomAlertViewStack alloc] init];
...
- (void)push:(CustomAlertView *)alertView
{
    for (CustomAlertView *av in self.alertViews) {
        if (av != alertView) {
            [av hide];
        }
        else {
...
- (void)pop:(CustomAlertView *)alertView
{
    ...
    CustomAlertView *last = [self.alertViews lastObject];
